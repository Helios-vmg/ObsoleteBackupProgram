...
EXPORT_THIS void test_func(){
	...
	cmp2.process();
	std::shared_ptr<rsync::NormalFile> file0(new rsync::NormalFile(paths[0]));
	std::shared_ptr<rsync::NormalFile> file1(new rsync::NormalFile(paths[1]));
	std::shared_ptr<rsync::NormalFile> file2(new rsync::NormalFile(paths[2]));
	std::shared_ptr<rsync::NormalFile> file3(new rsync::NormalFile(paths[3]));
